...
function H_Section_04() {
  ...
  // Auto-pause when user leaves the section
  useEffect(() => {
  ...
  // FIXED PROGRESS CLICK FUNCTIONALITY
  const handleProgressClick = (e) => {
    const video = videoRef.current;
    if (!video || !video.duration) return;

    const rect = e.currentTarget.getBoundingClientRect();
    const clickX = e.clientX - rect.left; // Exact horizontal click position
    const width = rect.width; // Total width of the bar
    
    // Calculate new time: (Percentage of bar clicked) * total duration
    const newTime = (clickX / width) * video.duration;
    video.currentTime = newTime;
    
    // Update progress state immediately for smooth UI feedback
    ...
    <section ref={sectionRef} className="w-full bg-[#00150F] text-white py-10 px-6">
      <div className="max-w-7xl mx-auto font-['Anek_Telugu'] text-center">
        ...
        <div className="relative text-center px-6">
          ...
          >
            ...
            <p className="text-white font-['Montserrat_Alternates'] max-w-lg font-medium mx-auto text-[20px] opacity-90 ">
              ...
              training is compliant with India’s securities regulations.”
            </p>
          </motion.div>
        ...
        <motion.div className="relative mt-6 rounded-3xl overflow-hidden shadow-2xl border border-green-900 group">
          ...
          >
            <source src="https://res.cloudinary.com/dr8poybri/video/upload/v1772915946/SanskarAcademy_t3wgyq.mp4" type="video/mp4" />
          </video>

          {/* Aesthetic Play/Pause Overlay */}
          <div className={`absolute inset-0 flex items-center justify-center transition-opacity duration-300 pointer-events-none z-10 ${isPlaying ? "bg-black/10 opacity-0 group-hover:opacity-100" : "bg-black/40 opacity-100"}`}>
            <motion.div
              animate={{ scale: isPlaying ? 0.8 : 1, opacity: isPlaying ? 0 : 1 }}
              className="w-20 h-20 flex items-center justify-center rounded-full bg-green-400/20 backdrop-blur-md border border-green-400/50"
            >
              {isPlaying ? (
                <svg className="w-8 h-8 text-green-400" fill="currentColor" viewBox="0 0 24 24"><path d="M6 4h4v16H6zm8 0h4v16h-4z" /></svg>
              ) : (
                <svg className="w-8 h-8 text-green-400 ml-1" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg>
              )}
            </motion.div>
          </div>
          ...
          <div
            ...
            className="absolute bottom-0 left-0 w-full h-3 bg-white/10 cursor-pointer z-30 group/progress flex items-end"
          >
            <div
